...
function initDashboardFilters() {
  ...
  form.addEventListener("submit", (evt) => {
    ...
    const person = data.get("person")?.toLowerCase();
    const filtered = state.operations.filter((operation) => {
      const date = operation.date;
      const matchFrom = from ? date >= from : true;
      const matchTo = to ? date <= to : true;
      const matchType = type && type !== "all" ? operation.type === type : true;
      const matchPerson = person ? operation.person.toLowerCase().includes(person) : true;
      return matchFrom && matchTo && matchType && matchPerson;
    });
